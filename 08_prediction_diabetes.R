...
train.index <- createDataPartition(data_folds$Group, p = .80, list = FALSE, times = folds)
...
for( i in 1:folds ) {
    ...
    # confusion matrix
    cm <- confusionMatrix(factor(test_folds$Group, levels = c(0,1)), 
...
saveRDS(object = cm_fold, file = "gbm_files/gbm_stats_diabetes.RDS")
saveRDS(object = gmp_fold_rel, file = "gbm_files/gbm_best_tree_diabetes.RDS")

# Plot gbm ----------------------------------------------------------------

cm_fold <- readRDS(file = "gbm_files/gbm_stats_diabetes.RDS")
gmp_fold_rel <- readRDS(file = "gbm_files/gbm_best_tree_diabetes.RDS")
...
p_gbm_infl <- gmp_fold_rel[names(sort(gmp_fold_rel_means, decreasing = F)), ] %>% 
    ...
    # ggplot2::annotate(geom = "text", x = 4, y = 50,  
    #                   label = paste0("Accuracy: ",
    #                                  round( mean(cm_fold$Accuracy, na.rm = T), 2 ) ), 
    #                   hjust = 0, parse = TRUE) +
    # ggplot2::annotate(geom = "text", x = 3, y = 50,  
    #                   label = paste0("kappa: ",
